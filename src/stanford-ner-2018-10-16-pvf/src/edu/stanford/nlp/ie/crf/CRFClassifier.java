  ...
  private double[] makeDatumUsingEmbedding(List<IN> info, int loc, List<FeatureFactory<IN>> featureFactories, PaddedList<IN> pInfo, List<String> featuresC, List<Clique> windowCliques) {
    ...
    for (int currLoc = loc-2; currLoc <= loc+2; currLoc++) {
      ...
      if (currLoc >=0 && currLoc < info.size()) {
        ...
        if (embeddings.containsKey(word))
        ...
        else if (embeddings.containsKey(word.toLowerCase()))
          embedding = embeddings.get(word.toLowerCase());
	else if(word.equalsIgnoreCase("<s>")){
    	    embedding = new double[200];
    	    for(int i=0;i<embedding.length;i++)embedding[i]=0.3;
	}else if(word.equalsIgnoreCase("</s>")){
    	    embedding = new double[200];
    	    for(int i=0;i<embedding.length;i++)embedding[i]=0.7;
	}else if(word.matches("[-+%.0-9]+")){
    	    embedding = new double[200];
    	    for(int i=0;i<embedding.length;i++)embedding[i]=0.5;
	}
        else{
          //embedding = embeddings.get("UNKNOWN");
          System.out.println("Unknown embedding for "+word+"; will use WS");
          
          StringBuilder result = new StringBuilder();
          try{
          URL url = new URL("http://89.38.230.23/word_embeddings_spanish/ws/wordvectors_get.php?w1="+word.toLowerCase());
          ...
          if(data.length>200){
        	...
        	embedding[i]=Double.parseDouble(data[i+1]);
          }else{
    	    for(int i=0;i<embedding.length;i++)embedding[i]=1.0;
          }
        ...
        embedding = new double[200];
        for(int i=0;i<embedding.length;i++)embedding[i]=0.0;
      }
...
//        System.err.println("FF #" + i + ": " + ((NERFeatureFactory) featureFactory).describeDistsimLexicon()); // XXXX
   ...
   */
  public void readEmbeddingsData() throws IOException {
  ...
  public static CRFClassifier<CoreLabel> chooseCRFClassifier(SeqClassifierFlags flags) {
