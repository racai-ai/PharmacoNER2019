...
function isMatchWord($m,$w,$ignoreCase){
    ...
    if(strncasecmp($m,"::SUFFIX:",9)==0){
        $suff=substr($m,9);
        $ret= endsWith(mb_strtolower($w),mb_strtolower($suff));
        //echo "::SUFFIX: [".mb_strtolower($w)."] [".mb_strtolower($suff)."] [".$ret."]\n";
        return $ret;
    }
...
function isDebug($rule){
  if(isset($rule['debug'])) return $rule['debug'];
  return false;
}
...
function applyRules($data,&$rules){
    ...
    foreach($data as $str){
        ...
        if(isDebug($rule)){print_r($match);print_r($c);var_dump($is_match);}
                            ...
                            if(!$is_match){
                                if($match['min']<=$match['n_matches']){
                                    ...
                                    if($rule['current_match']>=count($rule['match'])){
                                        ...
                                        $rule['last_match']=$j;
                                        $repeat=false;
                                        if(isDebug($rule)){echo "REPEAT=FALSE 1\n";var_dump($rule);}
                                ...
                                if(isWildcard($c,$match) && $rule['current_match']<count($rule['match'])-1 && isMatch($c,$rule['match'][$rule['current_match']+1])){
                                    ...
                                    if($rule['current_match']>=count($rule['match'])){
                                        $rule['last_match']=$j;
                                        $repeat=false;
                                        if(isDebug($rule)){echo "REPEAT=FALSE 2\n";var_dump($rule);}
                                        displayMatch($currentSent,$rule,$i);
                                    }
                                }else{
                                    $rule['match'][$rule['current_match']]['n_matches']++;
                                    if(isDebug($rule)){echo "REPEAT=FALSE 3\n";var_dump($rule);}
                                }
                            }
